function nilaiNormalWAh(lebar_jalan) {
    let w_ah = 4;
    if (lebar_jalan >= 15) {
        w_ah = 6;
    }
...
function nilaiC(j, w_h, s) {
    let c = j * (w_h / s);
    return c;
}

function nilaiJ(j_0, f_hs, f_uk, f_g, f_p, f_bki, f_bka) {
    let j = j_0 * f_hs * f_uk * f_g * f_p * f_bki * f_bka;
    return j;
}

function nilaiFP(l_p, l, w_h) {
    let f_p = Math.round((l_p / 3) - (((l - 2) * ((l_p / 3) - w_h)) / l)) / w_h;
    return f_p;
}

function nilaiJ0(l_e) {
    let j_0 = 600 * l_e;
    return j_0;
}

function nilaiFHS(tipe_lingkungan, tipe_hambatan_samping, tipe_fase, rasio_kendaraan_tak_bermotor) {
    let f_hs = 0.0;
...
function nilaiFUK(jumlah_juta_penduduk_kota) {
...
function nilaiFBKI(r_bki) {
    return 1.0 - r_bki * 0.16;
}

function nilaiFBKA(r_bka) {
    return 1.0 - r_bka * 0.16;
}

function nilaiRBKI(q_bki, q_total) {
    return q_bki / q_total;
}

function nilaiRBKA(q_bka, q_total) {
    return q_bka / q_total;
}

function nilaiDJ(q, c) {
    return q / c;
}

function nilaiNQ(n_q1, n_q2) {
    return n_q1 + n_q2;
}

function nilaiNQ1(d_j, s) {
    let n_q1 = 0;
    if (d_j > 0.5) {
...
function nilaiNQ2(d_j, s, r_h, q) {
    return s * ((1 - r_h) / (1 - r_h * d_j)) * (q / 3600);
}

function nilaiPA(n_q, l_m) {
    return n_q * (20 / l_m);
}

function nilaiRKH(n_q, q, s) {
    return 0.9 * (n_q / (q * s)) * 3600;
}

function nilaiNKH(q, r_kh) {
    return q * r_kh;
}

function nilaiRKTB(q_ktb,q_kb) {
    return q_ktb / q_kb
}
